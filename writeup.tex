...
By Definition 4.16 Corollary 4.2 of Koller, we can create an undirected moral graph from $G$ denoted $M[G]$ such that it is an I-map for $P_{B}$. Therefore,
...
\item [MEMM] Yes. In the v-structure at $S_{3,1} \rightarrow Y_{3} \leftarrow Y_{2}$ influence can flow from
...
\begin{eqnarray*}
 $P(x^{1} \mid z^{1}) \textless P(x^{1} \mid y^{1}, z^{1})$ \textbf{ (1)} \\
 $P(x^{1}, z^{1})/P(Z^{1}) \textless P(x^{1}, y^{1}, z^{1}) / {P(y^{1}, z^{1})$ \textbf{ (2)}\\
 $P(x^{1}, z^{1}) * P(y^{1}, z^{1})  \textless P(x^{1}, y^{1}, z^{1}) * P(z^{1})$ \textbf{ (3)} \\\\
 $(P(x^{1}, y^{0}, z^{1}) + P(x^{1}, y^{1}, z^{1})) * (P(x^{0}, y^{1}, z^{1}) + p(x^{1}, y^{1}, z^{1})  \textless \textbf{ (4)}\\
(P(x^{1}, y^{1}, z^{1})) * (P(x^{0}, y^{0}, z^{1}) + P(x^{1}, y^{0}, z^{1}) + P(x^{0}, y^{0}, z^{1}) + P(x^{1}, y^{1}, z^{1}))$ \\
\\
P(x^{1}, y^{0}, z^{1}) P(x^{0}, y^{1}, z^{1}) + P(x^{1}, y^{1}, z^{1}) P(x^{0}, y^{1}, z^{1}) \textbf{ (5)}\\
...
P(x^{0}, y^{0}, z^{1}) P(x^{1}, y^{1}, z^{1}))$ \textbf{ (6)}
\\
\\
P(x^{1},y^{0}, z^{1}) P(x^{0}, y^{1}, z^{1}) \textless  0$ \textbf{ (7)}\\
\end{eqnarray*}
...
\\
But it cannot be that $P(x^{1},y^{0}, z^{1}) P(x^{0}, y^{1}, z^{1}) \textless 0$ as both terms must either be 0, or greater than 0. Thus our original assumption is false, and it must be the case that
...
Perhaps a solution to this problem begins by marginalizing out the variables $l + 1$ to $k$.

\end{answer}

\begin{answer}{2.5.2.b}

It seems that the result would be inexact. If we have some v-structure such as $D_{1} \rightarrow F \leftarrow D_{2}$ and we remove $D_{2}$ then
we should have a dependency between $D_{1}$ and $D_{2}$ which no longer exists. 
